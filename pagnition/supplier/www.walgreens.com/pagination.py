...
def pagnition():
    ...
    tmpList=['url']
    template='https://www.walgreens.com/store/store/category/productlist.jsp?No=PageNumber&N=Cnumber&Eon=Cnumber'
    for url, count in zip(df['Category URL'], df['Product Count']):
        ...
        for i in range(pageNum):
            if(urltmp):
                tmpurl=template.replace('Cnumber',urltmp[0][1]).replace('PageNumber',str(i*24))
