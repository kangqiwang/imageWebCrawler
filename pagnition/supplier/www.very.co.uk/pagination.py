...
def pagnition():
    df=pd.read_csv("pagnition/input/very_co.csv",usecols=['Category URL","Category Name","First Page","Last Page","Source Domain","Product Count'])
    ...
    for url in df['Category URL","Category Name","First Page","Last Page","Source Domain","Product Count']:
        ...
        for i in range(pageNum):
            # if urltmp !='':
            tmpurl = urltmp+'?pageNumber='+str(i+1)+'&numProducts=99'
            # else:
            #tmpurl = url +'?Nao='+str(21*(i+1))+'&Ns=None&storeSelection=2408,2414,2409,2407,2404'
            print(tmpurl)
            tmpList.append(tmpurl)
    savedf=pd.Series(tmpList)
    savedf.to_csv("pagnition/output/very_co.csv",index=False)
