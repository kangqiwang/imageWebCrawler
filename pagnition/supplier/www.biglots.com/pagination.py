...
def pagnition():
    df=pd.read_csv("pagnition/input/biglots_com.csv",usecols=['Category URL','Product Count'])
    ...
    for url, count in zip(df['Category URL'],df['Product Count']):
        ...
        for i in range(pageNum):
            # if urltmp !='':
            urltmp = url+'?No='+str(perpageCount*i)+'&Nrpp=99'
            # else:
            #     tmpurl = url +'?Nao='+str(21*(i+1))+'&Ns=None&storeSelection=2408,2414,2409,2407,2404'
            print(urltmp)
            tmpList.append(urltmp)
    savedf=pd.Series(tmpList)
    savedf.to_csv("pagnition/output/biglots_com.csv",index=False)
